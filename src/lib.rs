...
fn get_client(ssl_path: &str) -> Client {
    ...
    Client::builder().connector(Connector::new().ssl(connector).finish()).finish()
}

fn get_url(host: &str, port: u32, request_uri: &str) -> String {
    format!("https://{host}:{port}/{request_uri}", host = host, port = port, request_uri = request_uri)
}
...
impl FullnodeClient {
    ...
    pub fn new(host: &str, port: u32, ssl_path: &str) -> Self {
        FullnodeClient {client: get_client(ssl_path), host: host.to_string(), port: port}
    }
    ...
    pub async fn get_blockchain_state(&self) -> Result<BlockchainState, Box<dyn Error>> {
        ...
        let mut resp:ClientResponse<Decompress<Payload>> = self.client.post(url).send_body("{}").await?;
        match resp.status(){
            StatusCode::OK => {
                let body: Bytes = resp.body().limit(1024 * 1024 * 50).await?;
                let body_str: &str = std::str::from_utf8(&body)?;
                let json_result: Result<responses::BlockchainStateResp, serde_json::Error> = serde_json::from_str(body_str);
    ...
    pub async fn get_block(&self, header_hash: String) -> Result<FullBlock, Box<dyn Error>> {
        ...
        let mut resp:ClientResponse<Decompress<Payload>> = self.client.post(url).send_body(serde_json::to_string(&request_body)?).await?;
        match resp.status(){
            StatusCode::OK => {
                let body: Bytes = resp.body().limit(1024 * 1024 * 50).await?;
                let body_str: &str = std::str::from_utf8(&body)?;
    ...
    pub async fn get_blocks(&self, start: u32, end: u32, exclude_header_hash: bool) -> Result<Vec<FullBlock>, Box<dyn Error>> {
        ...
        request_body.insert("exclude_header_hash".to_string(), json!(if exclude_header_hash {"True"} else {"False"}));
        let mut resp:ClientResponse<Decompress<Payload>> = self.client.post(url).send_body(serde_json::to_string(&request_body)?).await?;
        match resp.status(){
            StatusCode::OK => {
                let body: Bytes = resp.body().limit(1024 * 1024 * 50).await?;
    ...
    pub async fn get_all_blocks(&self, start: u32, end: u32) -> Result<Vec<FullBlock>, Box<dyn Error>> {
        self.get_blocks(start, end, true).await
    }
    pub async fn get_block_record_by_height(&self, height: u32) -> Result<BlockRecord, Box<dyn Error>> {
    ...
    pub async fn get_block_records(&self, start: u32, end: u32) -> Result<Vec<BlockRecord>, Box<dyn Error>> {
    ...
    pub async fn get_unfinished_block_headers(&self) -> Result<Vec<UnfinishedBlock>, Box<dyn Error>> {
        let url: String= get_url(self.host.as_str(), self.port, "get_unfinished_block_headers");
        let mut resp:ClientResponse<Decompress<Payload>> = self.client.post(url).send_body("{}").await?;
        match resp.status(){
            StatusCode::OK => {
                let body: Bytes = resp.body().limit(1024 * 1024 * 50).await?;
                let body_str: &str = std::str::from_utf8(&body)?;
                let json: responses::UnfinishedBlockAryResp = serde_json::from_str(body_str).unwrap();
                if json.success {
                    Ok(
                        json.headers
                    )
    ...
    pub async fn get_network_space(&self, older_block_header_hash: &String, newer_block_header_hash: &String) -> Result<u64, Box<dyn Error>> {
        let url: String= get_url(self.host.as_str(), self.port, "get_network_space");
        let mut request_body = Map::new();
        request_body.insert("older_block_header_hash".to_string(), json!(older_block_header_hash));
        request_body.insert("newer_block_header_hash".to_string(), json!(newer_block_header_hash));
        let mut resp:ClientResponse<Decompress<Payload>> = self.client.post(url).send_body(serde_json::to_string(&request_body)?).await?;
        match resp.status(){
            StatusCode::OK => {
                let body: Bytes = resp.body().limit(1024 * 1024 * 50).await?;
    ...
    pub async fn get_network_space_by_height(&self, older_block_height: u32, newer_block_height: u32) -> Result<u64, Box<dyn Error>> {
        ...
        self.get_network_space(&older_block.header_hash, &newer_block.header_hash).await
    }
    pub async fn get_additions_and_removals(&self, header_hash: String) -> Result<(Vec<CoinRecord>,Vec<CoinRecord>), Box<dyn Error>> {
        ...
        match resp.status(){
            StatusCode::OK => {
                let body: Bytes = resp.body().limit(1024 * 1024 * 50).await?;
                let body_str: &str = std::str::from_utf8(&body)?;
                let json: responses::AdditionsAndRemovalsResp = serde_json::from_str(body_str).unwrap();
                if json.success {
                    Ok(
                        (json.additions, json.removals)
                    )
                } else {
                    Err("Bad Failed to Fetch FullBlock".into())
                }
    ...
    pub async fn get_initial_freeze_period(&self) -> Result<u64, Box<dyn Error>> {
        ...
        match resp.status(){
            StatusCode::OK => {
                let body: Bytes = resp.body().limit(1024 * 1024 * 50).await?;
                let body_str: &str = std::str::from_utf8(&body)?;
                let json: responses::InitialFreezePeriodResp = serde_json::from_str(body_str).unwrap();
                if json.success {
                    Ok(
                        json.initial_freeze_end_timestamp
                    )
                } else {
                    Err("Bad Failed to Fetch FullBlock".into())
                }
    ...
    pub async fn get_network_info(&self) -> Result<NetworkInfo, Box<dyn Error>> {
        ...
        match resp.status(){
            StatusCode::OK => {
                ...
                if json.success {
                    Ok(
                        NetworkInfo::new(json.network_name, json.network_prefix)
                    )
                } else {
                    Err("Bad Failed to Fetch FullBlock".into())
                }
    ...
    pub async fn get_recent_signage_point_or_eos(&self, sp_hash: Option<String>, challenge_hash: Option<String>) -> Result<SignagePointOrEOS, Box<dyn Error>> {
        let url: String= get_url(self.host.as_str(), self.port, "get_recent_signage_point_or_eos");
        let mut request_body = Map::new();
        if sp_hash != None {
        ...
        if sp_hash != None && challenge_hash != None {
            ...
            match resp.status() {
                StatusCode::OK => {
                    let body: Bytes = resp.body().limit(1024 * 1024 * 50).await?;
                    let body_str: &str = std::str::from_utf8(&body)?;
                    let json: responses::SignagePointOrEOSResp = serde_json::from_str(body_str).unwrap();
                    if json.success {
                        Ok(
                            SignagePointOrEOS::new(json.signage_point, json.eos, json.time_received, json.reverted)
                        )
                    } else {
                        Err("Bad Failed to Fetch FullBlock".into())
                    }
    ...
    pub async fn get_coin_records_by_puzzle_hash(&self, puzzle_hash: String, include_spent_coins: bool, start_height: u32, end_height: u32) -> Result<Vec<CoinRecord>, Box<dyn Error>> {
        let url: String= get_url(self.host.as_str(), self.port, "get_coin_records_by_puzzle_hash");
        let mut request_body = Map::new();
        request_body.insert("puzzle_hash".to_string(), json!(puzzle_hash));
        request_body.insert("include_spent_coins".to_string(), json!(include_spent_coins));
    ...
    pub async fn get_coin_records_by_puzzle_hashes(&self, puzzle_hashes: Vec<String>, include_spent_coins: bool, start_height: u32, end_height: u32) -> Result<Vec<CoinRecord>, Box<dyn Error>> {
        let url: String= get_url(self.host.as_str(), self.port, "get_coin_records_by_puzzle_hashes");
        let mut request_body = Map::new();
    ...
    pub async fn get_coin_record_by_name(&self, name: String) -> Result<CoinRecord, Box<dyn Error>> {
    ...
    pub async fn get_coin_records_by_parent_ids(&self, parent_ids: Vec<String>, include_spent_coins: bool, start_height: u32, end_height: u32) -> Result<Vec<CoinRecord>, Box<dyn Error>> {
        let url: String= get_url(self.host.as_str(), self.port, "get_coin_records_by_parent_ids");
        let mut request_body = Map::new();
    ...
    pub async fn get_puzzle_and_solution(&self, coin_id: String, height: u32) -> Result<CoinSpend, Box<dyn Error>> {
    ...
    pub async fn get_coin_spend(&self, coin_record: &CoinRecord) -> Result<CoinSpend, Box<dyn Error>> {
        self.get_puzzle_and_solution(coin_record.coin.name().await?, coin_record.spent_block_index).await
    }
    ...
    pub async fn get_all_mempool_items(&self) -> Result<HashMap<String,MemPoolItem>, Box<dyn Error>> {
    ...
    pub async fn get_mempool_item_by_tx_id(&self, tx_id: &str) -> Result<MemPoolItem, Box<dyn Error>> {
...
impl WalletClient {
    pub fn new(host: &str, port: u32, ssl_path: &str) -> Self {
        WalletClient { client: get_client(ssl_path), host: host.to_string(), port: port }
    }
    ...
    pub async fn get_wallet_balance(&self, wallet_id: u32) -> Result<Vec<WalletBalance>, Box<dyn Error>> {
    ...
    pub async fn send_transaction(&self, wallet_id: u32, amount: u64, address: String, fee: u64) -> Result<TransactionRecord, Box<dyn Error>> {
        ...
        match resp.status(){
            StatusCode::OK => {
                let body: Bytes = resp.body().limit(1024 * 1024 * 50).await?;
                let body_str: &str = std::str::from_utf8(&body)?;
                let json: responses::TransactionRecordResp = serde_json::from_str(body_str).unwrap();
                Ok(
                    json.transaction
                )
            },
    ...
    pub async fn send_transaction_multi(&self, wallet_id: u32, additions: Vec<PendingPayment>, fee: u64) -> Result<TransactionRecord, Box<dyn Error>> {
    ...
    pub async fn get_transaction(&self, wallet_id: u32, transaction_id: String) -> Result<TransactionRecord, Box<dyn Error>> {
    ...
    pub async fn create_signed_transaction(&self, wallet_id: u32, additions: Vec<Coin>, coins: Vec<Coin>, fee: u64) -> Result<TransactionRecord, Box<dyn Error>> {
        ...
        match resp.status(){
            StatusCode::OK => {
                let body: Bytes = resp.body().limit(1024 * 1024 * 50).await?;
                let body_str: &str = std::str::from_utf8(&body)?;
                let json: responses::SignedTransactionRecordResp = serde_json::from_str(body_str).unwrap();
                Ok(
                    json.signed_tx
                )
            },
